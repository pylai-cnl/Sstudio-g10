...
import { 
  ...
  Heart as HeartIcon, ShieldCheck
} from "lucide-react";
...
export default function ProductDetailView({ 
  ...
  // 【修复点】：通过 users 字典安全地查找当前用户的 profile，判断其是否为超级管理员
  const currentUserProfile = currentUser ? users[currentUser.uid] : null;
  const isSuperAdmin = currentUser?.email === "relo@relo.com" || currentUserProfile?.isAdmin;
  
  // 核心拦截：验证上帝视角。哪怕不是 isOwner，只要是管理员，canEdit 统统为 true
            ...
            <>
              <div className="space-y-2">
                <div className="flex justify-between items-start gap-4">
                  <h1 className="text-2xl font-black text-gray-900 leading-tight flex-1 flex flex-wrap items-center gap-2">
                    {product.title}
                    {seller?.isAdmin && (
                      <span className="bg-orange-500 text-white px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-gray-900 flex items-center gap-2">
                  {sellerName}
                  {seller?.isAdmin && (
                    <span className="bg-black text-white px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest shadow-sm">
                      Relo Moderator
                    </span>
                  )}
                </h4>
                <p className="text-xs text-gray-400">
                  {seller?.isAdmin ? "Platform Administrator" : (sellerIsStudent ? "Verified Student • Cornell" : "Community Member")}
                </p>
              </div>
            </div>
            ...
            <div className="pt-4">
              {product.status === "Still on" ? (
                ...
                <div className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-bold text-gray-500">You control this item.</p>
              ...
              {product.status === "Still on" ? (
                ...
                <div className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl flex flex-col items-center justify-center gap-4">
                  ...
                  {product.status === "Pending" && (
                    <p className="text-xs text-gray-500 text-center italic">Waiting for the seller to deliver the item...</p>
                  )}
                  {(product.status === "Completed" || product.status === "Sold") && (
                ...
                <div className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl flex flex-col items-center justify-center gap-3">
                  <StatusBadge status={product.status} />
                  <p className="text-sm font-bold text-gray-500">This item is no longer available.</p>
                </div>
              )}
            </div>
          )}
